...
export class CompareByTenantComponent {
  ...
  onTenant1Change(): void {
    if (this.selectedTenant1) {
      this.selectedTenant2 = '';
      this.tenants = this.tenants.filter(tenant => tenant !== this.selectedTenant1);
    }
  }

  onTenant2Change(): void {
    if (this.selectedTenant2 === this.selectedTenant1) {
      this.snackBar.open('Tenant 2 cannot be the same as Tenant 1.', 'Close', {
        duration: 3000,
        panelClass: ['custom-toast', 'toast-error'],
        horizontalPosition: 'center',
        verticalPosition: 'top',
