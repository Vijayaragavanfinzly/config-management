...
@Component({
  ...
  standalone: true,
  imports: [RouterModule,CommonModule,FormsModule,NgSelectComponent,MatTabsModule],
  ...
  tenants: any[] = [];
    ...
    compareEnvironments(): void {
      if ( this.selectedEnv1 && this.selectedEnv2) {
        ...
        this.compareService.compareEnvironments( this.selectedEnv1.toLowerCase(), this.selectedEnv2.toLowerCase()).subscribe({
          next: (data) => {
    ...
    openConfirmationDialog(targetTenant: string, targetEnvironment: string, propertyValue: string, propertyKey: string) {
      ...
      dialogRef.afterClosed().subscribe(result => {
        ...
        if (result) {
          const payload = {
            targetTenant,targetEnvironment,propertyKey,propertyValue
          }
